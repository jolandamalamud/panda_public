function [track,l,dl,dsurr] = ll4b2a2epxb(x,D,mu,nui,doprior,options);
% 
...
alfa 		= 1./(1+exp(-x(5:6)));		% learning rate
epsilon 	= exp(x(7:8));				% 'pavlovian' parameter. Weigth of Vcue into Qgo
g    		= 1/(1+exp(-x(9)));		% irreducible noise 
...
for t=1:length(a)
	...
	if dodiff
		...
		dl(6-rho) = dl(6-rho) + g*(p0(a(t)) * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
		...
		dl(8-rho) = dl(8-rho) + g*(p0(a(t))*epsilon(2-rho)*V(s(t)) * ((a(t)==1)-p0(1))) / pg(a(t));

		dl(9) = dl(9) + g*(1-g)*(p0(a(t))-1/2)/pg(a(t));
