function [track,l,dl,dsurr] = ll2b2a2epxbu(x,D,mu,nui,doprior,options);
% 
...
for t=1:length(a)
	...
	q = Q(:,s(t)) .* C(:,s(t));
	q(1) = q(1) + epsilon(2-rho) * V(s(t)) * sum(C(:,s(t))) + bias;  
	...
	if dodiff
		for k=1:2
			tmp = (dQdb(:,s(t),k).* C(:,s(t)) + [epsilon(2-rho)*dVdb(s(t),k)*sum(C(:,s(t)));0]);
		...
		tmp = (dQde(:,s(t)).* C(:,s(t)) + [epsilon(2-rho)*dVde(s(t))*sum(C(:,s(t)));0]);
		...
		dl(6-rho) = dl(6-rho) + g*(p0(a(t))*epsilon(2-rho)*V(s(t))*sum(C(:,s(t))) * ((a(t)==1)-p0(1))) / pg(a(t));
    ...
    C(a(t),s(t)) = C(a(t),s(t)) + 1;
    track.go(:,t) = q; track.P(:,t) = pg; track.Q(:,:,t) = Q; track.V(:,t) = V; track.C(:,:,t) = C;
