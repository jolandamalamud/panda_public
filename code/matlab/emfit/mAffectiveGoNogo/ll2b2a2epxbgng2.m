function [l,dl,dsurr] = ll2b2a2epxbgng2(x,D,mu,nui,doprior,options);
% 
...
for t=1:length(a)
    ...
    rat = sum((r(t) == 1 && rho == 1) || (r(t) == 0 && rho == 0));
%     rat = rho;
	...
	if dodiff
        ...
        for k=1:2
            ...
            tmp = (2-rat) == k;
            dQde(a(t),s(t),k) = (1-alfa(2-rat))*dQde(a(t),s(t),k) + (er-Q(a(t),s(t)))*alfa(2-rat)*(1-alfa(2-rat)) * tmp;
            dVde(     s(t),k) = (1-alfa(2-rat))*dVde(     s(t),k) + (er-V(     s(t)))*alfa(2-rat)*(1-alfa(2-rat)) * tmp;
        end
            
%         tmp = (dQde(:,s(t)) + [epsilon(2-rho)*dVde(s(t));0]);
%         dl(4-rat) = dl(4-rat) + g*(p0(a(t)) * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
