function [track,l,dl,dsurr] = llbaepxb_test(x,D,mu,nui,doprior,options);
% 
...
for t=1:length(a)
	...
	q = Q(:,s(t)); 
	q(1) = q(1) + epsilon * (V(s(t))+vbias) + bias;    % add Pavlovian effect 
	...
	if dodiff
		...
		dl(3) = dl(3) + g*(p0(a(t))*epsilon*(V(s(t))+vbias) * ((a(t)==1)-p0(1))) / pg(a(t));
        ...
        tmp = [1;0];
		dl(6) = dl(6) + g*(p0(a(t)) * epsilon * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
	end
