function [l,dl,dsurr] = ll2b2a2epxbdepl(x,D,mu,nui,doprior,options);
% 
...
for t=1:length(a)
	...
	q = Q(:,s(t));
    pav = exp(epsilon(2-rho) + (1-rho) * sess(t) * delta);
    
    q(1) = q(1) + pav * V(s(t)) + bias; 
	...
	if dodiff
        ...
        dl(9) = dl(9) + g*(p0(a(t)) * (1-rho) * sess(t) * pav * V(s(t)) * ((a(t)==1)-p0(1))) / pg(a(t));
