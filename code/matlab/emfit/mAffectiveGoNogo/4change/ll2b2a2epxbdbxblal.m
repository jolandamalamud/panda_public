function [l,dl,dsurr] = ll2b2a2epxbdbxblal(x,D,mu,nui,doprior,options);
% 
...
dodiff=nargout==2;
np = length(x);
beta 		= x(1:2);                   % sensitivity to rewards and losses
alfa 		= x(3:4);		% learning rate
...
for t=1:length(a)
    ...
    if ismember(t, rep)
       [Q, V, dQdb, dVdb, dQde, dVde, dQdd, dVdd, dQdde, dVdde] = init_Qvalue();
    end
    ...
    psi = exp(beta(2-rho) + (1-rho) * sess(t) * deltabl);
	er = psi * r(t);
    
    lr = 1./(1+exp(-alfa(2-rho) - (1-rho) * sess(t) * deltaal));
	...
	if dodiff
        ...
        tmp = (dQdd(:,s(t)) + [epsilon(2-rho)*dVdd(s(t));0]);
        dl(11) = dl(11) + noise*(p0(a(t)) * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
        dQdd(a(t),s(t)) = (1-lr)*dQdd(a(t),s(t)) + lr * (1-rho) *  sess(t) * psi * r(t);
		dVdd(     s(t)) = (1-lr)*dVdd(     s(t)) + lr * (1-rho) * sess(t) * psi * r(t);
        ...
        tmp = (dQdde(:,s(t)) + [epsilon(2-rho)*dVdde(s(t));0]);
		dl(12) = dl(12) + noise*(p0(a(t)) * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
		dQdde(a(t),s(t)) = (1-lr)*dQdde(a(t),s(t)) + (er-Q(a(t),s(t)))*((1-rho) * sess(t) * lr)*(1-lr);
		dVdde(     s(t)) = (1-lr)*dVdde(     s(t)) + (er-V(     s(t)))*((1-rho) * sess(t) * lr)*(1-lr);
