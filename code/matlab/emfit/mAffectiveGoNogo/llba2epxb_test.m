function [l,dl,dsurr] = llba2epxb_test(x,D,mu,nui,doprior,options);
% 
...
for t=1:length(a)
	...
	q = Q(:,s(t)); 
	q(1) = q(1) + epsilon(2-rho) * (V(s(t)) + vbias) + bias;    % add Pavlovian effect 
	...
	if dodiff
		...
		dl(4-rho) = dl(4-rho) + g*(p0(a(t))*epsilon(2-rho)*(V(s(t)) + vbias) * ((a(t)==1)-p0(1))) / pg(a(t));
        ...
        tmp = [1;0];
		dl(7) = dl(7) + g*(p0(a(t)) * epsilon(2-rho) * (tmp(a(t)) - p0'*tmp)) / pg(a(t));
	end
