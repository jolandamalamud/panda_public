function [l,dl,dsurr] = ll2b2a2epxbm(x,D,mu,nui,doprior,options)
% 
...
m           = 1./(1+exp(-x(9)));        % forgetting parameter
...
for t=1:length(a)
	...
	if dodiff
		for k=1:2
            ...
            dQdb(~(actions==a(t)),:,k) = m*dQdb(~(actions==a(t)),:,k);
            dQdb(actions==a(t),~(stimuli==s(t)),k) = m*dQdb(actions==a(t),~(stimuli==s(t)),k);
            dVdb(~(stimuli==s(t)),k)= m*dVdb(~(stimuli==s(t)),k);
        end
        ...
        dQdm(a(t),s(t)) = (1-alfa(2-rho))*dQdm(a(t),s(t));   
