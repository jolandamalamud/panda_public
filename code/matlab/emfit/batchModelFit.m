function batchModelFit(Data,models,opt)
% 
...
if ~isfield(opt,'nograd');          opt.nograd = 0;         end
...
for mdl = 1:length(models)
    ...
    regressors = cell(models(mdl).npar,1);
    if isfield(opt,'reg'); 
        regressors{6} = opt.reg;
    end
	savestr = sprintf('%s/%s',opt.resultsDir,models(mdl).name);
	[params,var,alpha,stats,bf,fitparams] = emfit(models(mdl).name,Data,models(mdl).npar,regressors,[],[],opt.nograd,opt.maxit,[],savestr); 
